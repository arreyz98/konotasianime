...
const DetailTabs = ({dataVideo} : DataVideoProps) => {
    ...
    <div className="flex flex-col w-full sm:w-[80%] h-full mx-auto bg-black rounded-2xl">
      ...
      {currentTabs == "episodes" ? (
        <div className="flex flex-row space-x-1.5 p-5 h-[200px]">
          ...
          <Button onClick={() => handleSelectVideo(index)} key={item.id} className={`${selectedVideo == index ? "bg-blue-500 hover:bg-blue-500" : "bg-[#1C2029]"} cursor-pointer`}>{item.episode}</Button>
          ))}
        ...
        (<div className="p-6">
          ...
          <div className="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
            ...
            <dd className="text-gray-700 sm:col-span-2">
              <Badge className="bg-blue-500  px-4 text-base">{data[selectedVideo].episode}</Badge>
            </dd>
          </div>
          <div className="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
